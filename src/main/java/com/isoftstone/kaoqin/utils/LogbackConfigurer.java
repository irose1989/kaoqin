 ...
 * <p/>
 * Only needed for non-default Logback initialization with a custom config location. By default, Logback will simply
 * read its configuration from a "logback.xml" or "logback_test.xml" file in the root of the classpath.
 * <p/>
 * For web environments, the analogous LogbackWebConfigurer class can be found in the web package, reading in its
 * configuration from context-params in web.xml. In a JEE web application, Logback is usually set up via
...
public class LogbackConfigurer {
     ...
     * 
     * @param location the location of the config file: either a "classpath:" location (e.g. "classpath:logback.xml"),
     * an absolute file URL (e.g. "file:C:/logback.xml), or a plain absolute path in the file system (e.g.
     * "C:/logback.xml")
     * @throws FileNotFoundException if the location specifies an invalid file path
     * @throws ch.qos.logback.core.joran.spi.JoranException Thrown
     */
     ...
     * <p/>
     * This can be used e.g. for test environments, for applications that leverage LogbackWebConfigurer's
     * "webAppRootKey" support in a web environment.
     * 
     * @param key system property key to use, as expected in Logback configuration (for example: "demo.root", used as
