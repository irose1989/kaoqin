...
/**
 * Convenience class that performs custom Logback initialization for web environments, allowing for log file paths
 ...
 * global too).
 * <p/>
 * Supports two init parameters at the servlet context level (that is, context-param entries in web.xml):
 * <ul>
 ...
 * "/WEB-INF/logback.xml"). If not specified, default Logback initialization will apply ("logback.xml" or
 * "logback_test.xml" in the class path; see Logback documentation for details).
 ...
 * application root directory. Default is "true"; specify "false" to suppress expose of the web app root system
 * property. See below for details on how to use this system property in log file locations.
 * </ul>
 * <p/>
 * Note: <code>initLogging</code> should be called before any other Spring activity (when using Logback), for proper
 * initialization before any Spring logging attempts.
 * <p/>
 * By default, this configurer automatically sets the web app root system property, for "${key}" substitutions within
 * log file locations in the Logback config file, allowing for log file paths relative to the web application root
 * directory. The default system property key is "webapp.root", to be used in a Logback config file like as follows:
 * <p/>
 * <code>
